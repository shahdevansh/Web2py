...
def search():
	results=[]
	x=request.vars['s']
	results = db(db.Item.Heading.contains(x) & (db.Item.id>=0) & (db.Item.User == db.auth_user.id) & (db.Category.id == db.Item.Category)).select(orderby='Item.Rating DESC')
...
def listall():
	cate = db(db.Category.id>0).select()
	x = db((db.Item.id>=0) & (db.Item.User == db.auth_user.id) & (db.Category.id == db.Item.Category)).select(orderby='Item.Rating DESC')
	return dict(x=x,cate=cate)
...
def list_cat():
	cate = db(db.Category.id>0).select()
	x = db((request.args(0)==db.Category.id) & (db.Item.id>=0) & (db.Item.User == db.auth_user.id) & (db.Category.id == db.Item.Category)).select(orderby='Item.Rating DESC')
...
def index():
    ...
    if check_access():
    	...
    	comments = db(db.Comments.AuthID == auth.user_id).select()
#    redirect(URL("listall"))
